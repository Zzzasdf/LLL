    ...
    IRecipient<ViewAllHideAsyncRequestEvent>
{
    ...
    {
        if (!viewTree.TryGetStackWithCheckTip(viewType, out Stack<ViewNode> stack))
        {
            return false;
        }
    ...
    private UniTask<bool> HideAsync_Internal(ViewEntityBase view)
    {
        int uniqueId = view.UniqueId;
        ViewType viewType = ViewUniqueIdGenerator.Default.GetViewType(uniqueId);
        Stack<ViewNode> stack = viewTree.GetStack(viewType);
        subViewLayerDriver.HideView(stack);
