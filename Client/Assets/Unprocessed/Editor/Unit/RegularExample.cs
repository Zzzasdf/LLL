...
public class RegularExample
{
    ...
    public void MatchHeadTailChar()
    {
        ...
        CDebug.Log(Regex.Replace(str, "^", "准备开始："));
        // 匹配结束 $
        string str2 = "I am Blue cat";
        CDebug.Log(Regex.Replace(str2, "$", "结束了！"));
    }
    ...
    public void MatchOutside()
    {
        ...
        CDebug.Log(("除ahou这之外的所有字符,原字符为:" + strFind1 + "替换后:" + Regex.Replace(strFind1, @"[^ahou]", "*")));
        CDebug.Log(("除ahou这之外的所有字符,原字符为:" + strFind2 + "替换后:" + Regex.Replace(strFind2, @"[^ahou]", "*")));
    }
    ...
    public void MatchQQ()
    {
        ...
        CDebug.Log((isQq1 + "是否为合法QQ号(5-12位数字):" + Regex.IsMatch(isQq1, regexQq)));
        CDebug.Log((isQq2 + "是否为合法QQ号(5-12位数字):" + Regex.IsMatch(isQq2, regexQq)));
        CDebug.Log((isQq3 + "是否为合法QQ号(5-12位数字):" + Regex.IsMatch(isQq3, regexQq)));
        CDebug.Log((isQq4 + "是否为合法QQ号(5-12位数字):" + Regex.IsMatch(isQq4, regexQq)));
    }
    ...
    public void MatchNumOrLetter()
    {
        ...
        CDebug.Log((findStr1 + "中的字母和数字组成的新字符串为:" + newStrFind));
    }
    ...
    public void MatchName()
    {
        ...
        Regex.Split(strSplit, regexSplitstr).ToList().ForEach(i => CDebug.Log(i));
    }
    ...
    public void MatchTelephoneNumber()
    {
        ...
        CDebug.Log(("电话号码 " + TelNumber1 + " 是否合法:" + RegexTelNumber3.IsMatch(TelNumber1)));
        CDebug.Log(("电话号码 " + TelNumber2 + " 是否合法:" + RegexTelNumber3.IsMatch(TelNumber2)));
        CDebug.Log(("电话号码 " + TelNumber3 + " 是否合法:" + RegexTelNumber3.IsMatch(TelNumber3)));
        CDebug.Log(("电话号码 " + TelNumber4 + " 是否合法:" + RegexTelNumber3.IsMatch(TelNumber4)));
        CDebug.Log(("电话号码 " + TelNumber5 + " 是否合法:" + RegexTelNumber3.IsMatch(TelNumber5)));
        CDebug.Log(("电话号码 " + TelNumber6 + " 是否合法:" + RegexTelNumber3.IsMatch(TelNumber6)));
        CDebug.Log( ("电话号码 " + TelNumber7 + " 是否合法:" + RegexTelNumber3.IsMatch(TelNumber7)));
    }
    ...
    public void MatchRepeat()
    {
        ...
        CDebug.Log(("单字符重复2两次替换为22, 结果为：" + Regex.Replace(inputStr, strGroup1, "22")));
        // 重复 多个字符 使用（ abcd ）{n}进行分组限定
        string strGroup2 = @"(ab\w{2}){2}";
        CDebug.Log(("分组字符重复2两次替换为5555, 结果为：" + Regex.Replace(inputStr, strGroup2, "5555")));
    }
    ...
    public void MatchIP()
    {
        ...
        CDebug.Log((inputStrIp4 + " 是否为合法的IP4地址:" + Regex.IsMatch(inputStrIp4, regexStrIp4)));
    }
    ...
    public void MatchZeroWidthAssertion()
    {
        ...
        CDebug.Log(reg.Match("dsA123A"));
        CDebug.Log(reg.Match("dsA123A").Groups[1]); // 捕获文本到自命名的数值里

        // (?<name>exp) 匹配exp, 并捕获文本到名称为name的数组里
        Regex reg2 = new Regex(@"A(?<num>\w+)A");
        CDebug.Log(reg2.Match("dsA123A").Groups["num"]); // 123
        Regex reg3 = new Regex(@"A(?:\w+A)");
        CDebug.Log(reg3.Match("dsA123A"));

        Regex reg4 = new Regex(@"sing(?=ing)"); // 在sing的后面会有ing, 如果sing后你那紧跟ing, 那么这个sing才匹配成功
        CDebug.Log(reg4.Match("ksingkksingingkkkk")); // single
        CDebug.Log(reg4.Match("singddddsingsingd").Index); // 0

        Regex reg5 = new Regex(@"(?<=wo)man"); // (?<exp) 匹配exp后面的位置
        CDebug.Log(reg5.Match("Hi man Hi woman")); // man
        CDebug.Log(reg5.Match("Hi man Hi woman").Index); // 12

        Regex reg6 = new Regex(@"sing(?!ing)"); // (?!exp) 匹配后面跟的不是exp的位置
        CDebug.Log(reg6.Match("singing-singabc")); //sing
        CDebug.Log(reg6.Match("singing-singabc").Index); // 8

        Regex reg7 = new Regex(@"(?<!wo)man"); // (?!exp) 匹配前面不是exp的位置
        CDebug.Log(reg7.Match("Hi woman Hi man")); // man
        CDebug.Log(reg7.Match("Hi woman Hi man").Index); // 12

        Regex reg8 = new Regex("ABC(?#这是一段注释。)DEF");
        CDebug.Log(reg8.Match("ABCDEF"));
    }
}
