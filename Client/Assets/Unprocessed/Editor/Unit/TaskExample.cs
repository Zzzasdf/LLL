...
public class TaskExample
{
    ...
    public void Status()
    {
        ...
        CDebug.Log(task.Status); // Created
        task.Start();
        CDebug.Log(task.Status); // WaitingToRun
        Thread.Sleep(1000);
        CDebug.Log(task.Status); // Runing
        Thread.Sleep(2000);
        CDebug.Log(task.Status); // RanToCompletion
        CDebug.Log($"Result => {task.Result}");
    }
    ...
    public async void TaskCancel()
    {
        ...
        try
        {
            ...
            Thread.Sleep(2000);
            cts.Cancel();
            CDebug.Log("await Before");
            await task;
            CDebug.Log("await After");
        }
        catch (TaskCanceledException)
        {
            CDebug.Log("Task canceled");
        }
        finally
        {
            cts.Dispose();
    ...
    public async void TaskCancelCollection()
    {
        ...
        async Task Example1()
        {
            ...
            CDebug.Log($"Task completed in {sw.ElapsedMilliseconds} ms");
        }
    ...
    public async void RegisterRehabilitate()
    {
        using (var cts = new CancellationTokenSource(TimeSpan.FromSeconds(3)))
        {
            cts.Token.Register(() => CDebug.Log("Cancelation requested in Main Thread"));
            try
            {
                cts.Token.Register(() => CDebug.Log("Cancelation requested in Try Block"));
    ...
    public async void TaskTimeout()
    {
        ...
        async Task FooAsync(CancellationToken? cancellationToken = null)
        {
            ...
            try
            {
                CDebug.Log("Foo start...");
                await Task.Delay(5000, token);
                CDebug.Log("Foo end...");
            }
            catch (OperationCanceledException)
            {
                CDebug.Log("Foo cancelled...");
                throw;
            }
        }
    }
    ...
    public async void ResolveFireAndForget()
    {
        ...
        void Extensions()
        {
            try
            {
                ...
                CDebug.Log("Loading data ...");
                Thread.Sleep(2000);
                var data = dataModel.Data;
                CDebug.Log(data);
                CDebug.Log($"Data is loaded: {dataModel.IsDataLoaded}");
            }
            catch (Exception e)
            {
                CDebug.Log(e);
        ...
        async void TaskFactory()
        {
            try
            {
                ...
                var data = dataModel.Data;
                CDebug.Log(data);
                CDebug.Log("Data is loaded finish");
            }
            catch (Exception e)
            {
                CDebug.Log(e);
    ...
    class SeanDataModel
    {
        ...
        private bool OnDataLoaded(Task t)
        {
            ...
            {
                // t.Exception.Message);
                CDebug.Log(t.Exception.InnerException.Message);
            }
