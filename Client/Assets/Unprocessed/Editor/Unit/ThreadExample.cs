...
public class ThreadExample
{
    ...
    public void BuiltInMethod()
    {
        ...
        CDebug.Log($"Elapsed time: {sw.ElapsedMilliseconds}ms");
    ...
    public void ThreadTermination()
    {
        ...
        void ThreadJoin()
        {
            var th = new Thread((object? obj) =>
            {
                ...
                {
                    Thread.Sleep(200);
                    CDebug.Log("Thread is still running...");
                }

                CDebug.Log("Thread is finished!");
            }) { IsBackground = true, Priority = ThreadPriority.Normal };
            th.Start(123);
            CDebug.Log("In main thread, waiting for thread to finish...");
            th.Join();
            CDebug.Log("Done.");
        }

        // 中断线程执行
        // 如果线程中包含一个 while(true) 循环(常见的后台线程)，那么需要保证包含等待方法（占用率满了，没有其他资源空闲出来抛出异常），如IO操作，Thread.Sleep等
        // Q: 不能使用Abort?
    ...
    public void LocksAndSemaphores()
    {
        ...
        void Semaphore()
        {
            ...
            CDebug.Log("Outputs:");
            CDebug.Log( string.Join(",", outputs));
            CDebug.Log($"Elapsed time: {sw.ElapsedMilliseconds}ms");
            ...
            void ReceiveMessageThread(object? id)
            {
                try
                {
                    while (true)
                    {
                        ...
                        CDebug.Log($"Thread {id} received {message.Content} from {message.FromId}");
                        Thread.Sleep(1);
                    }
                }
                catch (ThreadInterruptedException)
                {
                    CDebug.Log($"Thread {id} interrupted");
                }
            }
        }
